<style lang="less" scoped>
@import '~@/vant-variables.less';
.page {
  background-color: @gray-1;
}
.notifyBox {
  background-color: @white;
  padding: 20px;
  margin-bottom: 5px;
  &__top {
    display: flex;
    align-items: flex-start;
    font-size: 14px;
    &__avatar {
      margin-right: 5px;
    }
    &__date {
      font-size: 12px;
      color: @gray-4;
    }
  }
}
</style>
<template>
  <div class="page">
    <van-nav-bar id="toolbar" title="评论与回复" left-text="返回" left-arrow @click-left="back" fixed placeholder></van-nav-bar>
    <div v-for="(item,index) in commentList" :key="index" class="notifyBox">
      <div class="notifyBox__top">
        <div class="notifyBox__top__avatar">
          <v-username class="comment__avatar" type="avater" :avatarSize="28" :username="item.buildUsername"></v-username>
        </div>
        <div>
          <div>
            <v-username type="username" :username="item.buildUsername"></v-username>
            {{item.action=='NEW_COMMENT'?'评论':'回复'}}了你的评论
          </div>
          <div class="notifyBox__top__date">{{item.buildDate}}</div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import Username from '@/components/common/username/username';
import userNotifyStore from './userNotifyStore.js';

export default {
  components: {
    'v-username': Username,
    // 'v-fileBox': fileBox,
  },
  mounted() {
    userNotifyStore.pullUserNotifyList(this, () => {
      this.commentList = userNotifyStore.getUserNotifyList().commentList;
    });
  },
  data() {
    return {
      commentList: [],
    };
  },
  methods: {
    back() {
      this.$router.go(-1); //返回上一层
    },
  },
};
</script>

<style scoped>
</style>