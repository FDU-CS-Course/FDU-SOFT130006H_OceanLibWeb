            ...
            for (const element of notifyList) {
                if (element.username !== element.notifyEntity.buildUsername) {
                    switch (element.notifyEntity.action) {
                        ...
                        case "NEW_COMMENT":
                        case "NEW_REPLY":
                            if (element.notifyEntity.targetType === "NOTE") {
                                let replyNoteID = element.notifyEntity.targetID;
                                await self.$Axios({
                                    method: 'post',
                                    url: '/noteService/getNoteCommentById',
                                    params: {
                                    ...
                                    if (response.data.msg) {
                                        // 获取原评论内容
                                        originalComment = response.data.msg.content;
                                    }
